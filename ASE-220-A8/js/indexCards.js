...
function createPodCastCard(Info) 
{
    ...
    if (owned)
    {
        ...
        edit.id = "savebutton";
        edit.onclick = function() { setEdit(Info.DOI); location.href="podcasts/edit.html"; }
...
function createAuthorCard(Info) 
{
    ...
    unfollow.onclick = function() { UnFollow(Info.First + " " + Info.Last); }
...
function checkDisc(disc)
{
    for (let i = 0; i < disc.length; i++)
    {
        ...
        for (let j = 0; j < DiscArray.length; j++)
        {
            if (disc[i] == DiscArray[j])
                exists = true;
        } 

        if (!exists)
        {
            DiscArray.push(disc[i]);
            DiscArrayAMT.push(0);
        }
    }
}

function countDisc(disc)
{
    for (let i = 0; i < disc.length; i++)
    {
        var exists = false;
...
let initDisciplines = () => 
{
    ...
    $.getJSON(podcastDB, function(data)
    {
        ...
        for (let i = 0; i < data.length; i++) 
        {
            ...
            var DiscArr = seperateByComma(data[index]['disc']);
            checkDisc(DiscArr);
            countDisc(DiscArr);

            //console.log(DiscArray);
            //console.log(DiscArrayAMT);
        }
...
let initAuthors = () => 
{
    ...
    $.getJSON(userDB, function(data)
    {
        ...
        var indexOfAuthor = null;
        for (let i = 0; i < data.length; i++) 
        {
            if (getAuthor() == data[i]['First'] + " " + data[i]['Last'])
            {
                if (indexOfAuthor == null)
                    indexOfAuthor = i;
            }
        }
        ...
        if (indexOfAuthor != null)
        {
            ...
            var i = indexOfAuthor;
            localStorage.setItem('AuthorEmail', data[i]['email']);
            localStorage.setItem('AuthorSaved', JSON.stringify(data[i]['saved']));
...
let initTags = () => 
{
    ...
    $.getJSON(podcastDB, function(data)
    {
        ...
        for (let i = 0; i < data.length; i++) 
        {
            ...
            var DiscArr = seperateByComma(data[index]['tags']);
            checkTags(DiscArr);
            countTags(DiscArr);
...
else if (getCurrentPage() == 'categories.html')
{
    initDisciplines();
}
else if (getCurrentPage() == 'users.html')
{
    initAuthors();
}
else if (getCurrentPage() == 'keywords.html')
{
    initTags();
}
else
